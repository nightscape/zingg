...
/** End-to-end entry point.
  ...
  * }}}
  */
class Zingg(cfg: ZinggConf) extends Serializable {
  ...
  def findTrainingData(
    ...
    val sampled = priorModel match {
      ...
      case None =>
        ActiveLearning.coldStartSample(featured, n)
    }
  ...
  def interactiveSession(
    ...
    val collected = featured.collect()
    val rng       = new scala.util.Random(shuffleSeed)
    val shuffled  = rng.shuffle(collected.toIndexedSeq).toArray
    new InteractiveSession(cfg, featured, tree, shuffled, ec)
  }
